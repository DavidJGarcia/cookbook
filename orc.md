...
### 3.3 — Demo artifacts (if applicable)

For UI changes, capture screenshots or a short recording with the same tool you drove the app with in 3.2 (the Chrome / Preview / computer-use screenshot capture, or a GIF capture if one's available):

- Happy path
- Key edge cases
- Error states

For backend/CLI changes, capture representative command output, API responses, or log excerpts instead. Skip entirely if the change is internal-only (e.g. a refactor with no behavior change).
