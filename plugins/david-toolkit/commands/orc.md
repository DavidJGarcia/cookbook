...
**Watch by event, never by timer.** Within a turn, the background watch above is the mechanism. Across turns, `subscribe_pr_activity` is: subscribe once after cutting the PR, then end the turn — ending the turn *is* how you wait, and CI failures, review comments, and merge-conflict notices wake the session on their own. Do **not** arm a scheduled self check-in to re-poll the PR: no `send_later`, no `create_trigger` Routine, no `/loop`, no foreground `sleep`. A timed re-check that finds nothing changed spends a whole context window to produce no work, and it repeats for every open PR. `send_later` is denied in these repos' `.claude/settings.json`; see `.claude/skills/steward/SKILL.md` for the full posture. If events dry up and the PR goes quiet, say so in the handoff instead of arming a timer to cover it.
